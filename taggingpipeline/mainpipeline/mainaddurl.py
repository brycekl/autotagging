...
from get_info_from_url import get_tags,get_product_infp,upload_tag_res,get_products,get_total_num
import queue
...
def auto_label(servicetag,queueimg,version,resqt,resq,params=None,res_dir=None,):
    ...
    try:
        ...
        while canget:
            ...
            imgsinfo =  queueimg.get(block=True)
            ...
            if tag_perp != []:
                ...
                for each_tagres in tag_perp:
                    if each_tagres['label'] == 'first category':
                        ...
                        tag_res["tags"][each_tagres['label']]= each_tagres['value'] if type(each_tagres['value'])==list else [each_tagres['value']]
                ...
                resq.put(tag_res)
            message = 'no message'
            ifpost ,savenum,tmp_tagres_dir,resq,message = handle_tag_res(resq,maxlimit=20,outdir=tagres_outdir,if_min=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfgpath', type=str, default='/root/autodl-tmp/autotagging/start_tag.json', help='question.json path')
