                ...
                if not pd.isna(truth_value) :
                    ...
                    if (not pd.isna(pred_value)) : 
                        if  pred_value.lower() == truth_value.lower():
                            ...
                            error_res[product_id][col] = {'true':truth_value,'pred_value:':pred_value}
                            error_res[product_id]['first category value '] = {'true':row['first category_truth'],'pred_value:':row['first category_pred']}
                            error_res[product_id]['subcategory value'] = {'true':row['subcategory_truth'],'pred_value:':row['subcategory_pred']}
...
def merge_and_unique_columns(df, col1, col2, separator=','):
    ...
    return df.apply(lambda row: separator.join(set(str(row[col1]).split(separator) + str(row[col2]).split(separator))), axis=1)
...
# 合并 spu_1和spu_2
    ...
    merged_df.drop(['first category_1','first category_2','subcategory_1','subcategory_2','title.1_2'], axis=1, inplace=True)
    # 去掉列名为空的列
    merged_df = merged_df.dropna(axis=1, how='all')
    merged_df.rename(columns={'Season':'SEASON','Cut':'CUT','BACK STYLE':'BACK','title.1_1':'title'}, inplace = True)
    # 删掉第一行
...
if __name__ == "__main__":
    ...
    parser.add_argument('--truth_path', type=str, default='/root/autodl-tmp/autotagging/taggingpipeline/data/data_1/datav1_5_cloth.csv', help='url')
    parser.add_argument('--prediction_path', type=str, default='/root/autodl-tmp/tmp_res/tag_res/output/data1_6_1106_20231103_cl_20231109_0024/20231103_cl_20231109_0024_label_res_t.csv', help='res_dir')
    ...
    parser.add_argument('--resoutput_dir', type=str, default='/root/autodl-tmp/autotagging/taggingpipeline/test/acc/1109_1347', help='res_dir')
